...
import contactEmailTemplate from "../templates/bharatxinfratech.contact.js";
...
async function getZohoAccessToken() {
    const res = await fetch("https://accounts.zoho.in/oauth/v2/token", {
        ...
        body: new URLSearchParams({
            refresh_token: process.env.BHARATXINFRATECH_ZOHO_REFRESH_TOKEN,
            client_id: process.env.BHARATXINFRATECH_ZOHO_CLIENT_ID,
            client_secret: process.env.BHARATXINFRATECH_ZOHO_CLIENT_SECRET,
...
async function sendZohoEmail({ fromName, fromEmail, toEmail, subject, htmlContent }) {
    ...
    const accountId = process.env.BHARATXINFRATECH_ZOHO_ACCOUNT_ID;
...
router.post("/", async (req, res) => {
    ...
    try {
        await sendZohoEmail({
            fromName: "BharatX Infratech",
            fromEmail: process.env.BHARATXINFRATECH_EMAIL_USER,
            toEmail: process.env.BHARATXINFRATECH_EMAIL_TO,
...
export { router as bharatXInfratechContactRoute };
